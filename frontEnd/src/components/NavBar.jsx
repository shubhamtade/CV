...
const NavBar = ({ theme, setTheme, navBackground }) => {
  ...
  return (
    <nav className={`z-40 h-[60px] md:h-[80px] transition duration-300 w-[90%] md:max-w-[1050px] mx-auto flex`}>
      <div className="flex justify-between w-full items-center py-2 md:py-4">
        ...
        <div className="w-full flex gap-2 md:gap-4 items-center md:mx-0">
          <a href="#top" className="w-[60px] text-white md:hidden">
            <img
              src={theme === "light" ? (navBackground ? LogoIconWhite : LogoIconBlack) : LogoIconWhite}
              className="duration-100"
              alt="Logo Icon"
            />
          </a>
          {navBackground && (
            <p className="font-bold text-white md:hidden delay-3000">Shubham Sunil Tade</p>
          )}
          <a href="#top" className="hidden w-[80%] text-white md:block">
            <img
              src={theme === "light" ? (navBackground ? LogoWhite : LogoBlack) : LogoWhite}
              className="duration-100"
              alt="Logo"
            />
          </a>
        ...
        <div className="w-max h-full flex items-center">
          <ul className="w-max hidden md:flex items-center gap-12">
            <li className="h-full font-rubik font-bold cursor-pointer">
              <a href="#skills" className="h-full flex items-center hover:no-underline">
                Skills Acquired
              </a>
            </li>
            <li className="h-full font-rubik font-bold cursor-pointer">
              <a href="#projects" className="h-full flex items-center hover:no-underline">
                My Projects
              </a>
            </li>
            <li className="h-full font-rubik font-bold cursor-pointer">
            ...
            <li className="h-full font-rubik font-bold cursor-pointer border-b-4 border-[#ffba4a] hover:border-white hover:duration-300">
              <a className="h-full flex items-center" onClick={() => navigate("/auth")}>
                Signin / Signup
              </a>
            </li>
          </ul>
          <div className="pl-6 items-center justify-center md:justify-start gap-6 text-xl hidden md:flex ml-6 border-portfolio-highlighter border-l-[1px] border-[#0E43A0]">
            <a href="https://www.linkedin.com/in/shubham-tade" target="_blank" rel="noreferrer" className="hover:scale-110">
              <LinkedInIcon />
            </a>
            <a href="https://github.com/shubhamtade" target="_blank" rel="noreferrer" className="hover:scale-110">
              <GitHubIcon />
            </a>
            <div className="flex items-center justify-center w-full h-full hidden md:block">
              <div className="flex items-center justify-center w-full h-full">
                <label className={`swap swap-rotate ${theme === "dark" ? "animate-spin-slow" : ""}`}>
      ...
      {isMenuOpen && (
        <div className="fixed top-0 left-0 w-full h-full text-white bg-gradient-to-tr from-lime-400 via-emerald-500 to-teal-700 flex flex-col items-center justify-center z-50">
          <CloseIcon onClick={toggleMenu} className="absolute top-10 left-10 cursor-pointer" />
          <div className="flex flex-col items-left gap-2 absolute top-32 left-20">
            <a href="#home" className="text-2xl mb-4 px-5 py-2 rounded-full" onClick={toggleMenu}>
              Home
            </a>
            <a href="#about" className="text-2xl mb-4 px-5 py-2 rounded-full" onClick={toggleMenu}>
              Skills Acquired
            </a>
            <a href="#contact" className="text-2xl mb-4 px-5 py-2 rounded-full" onClick={toggleMenu}>
              My Projects
            </a>
            <a href="#contact" className="text-2xl mb-4 px-5 py-2 rounded-full" onClick={toggleMenu}>
              Job Experiences
            </a>
            <a className="text-2xl mb-4 bg-[#4883e9] px-5 py-2 rounded-full" onClick={() => navigate("/auth")}>
              Signin / Signup
            </a>
          </div>
